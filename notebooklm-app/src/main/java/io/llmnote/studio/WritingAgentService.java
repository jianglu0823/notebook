...
/**
 * 方案7:多智能体协同迭代写作。三个 ReActAgent 由本服务编排为迭代收敛循环:
 *  - 作者(author):写/改稿;
 *  - 核查(factchecker):挂 web_search 工具,ReAct 循环里自主联网核实事实、纠错;
 *  - 编辑(editor):审稿并给 REVISE/APPROVE 裁决(收敛信号)。
 * 每轮:核查 + 编辑给意见 → 若编辑 APPROVE 则收敛;否则作者据反馈改稿,最多 maxRounds 轮。
 * 每轮过程写入 rounds JSON,前端轮询「围观」协作。
 *
 * 与小红书那套顺序状态机的本质区别:核查员是真正的 agentic —— 由 LLM 自主决定
 * 是否/何时/以什么关键词调用 web_search 工具(ReAct 推理-行动循环),而非硬编码流程。
 */
@Slf4j
@Service
...
public class WritingAgentService {
    ...
    public WritingProject create(String ownerId, String topic, String genre, Integer maxRounds) {
    ...
    public void runAsync(Long id) {
        ...
        try {
            ...
            emit(p, eventsArr, "start", "system", "多智能体协作启动:作者写稿 → 核查员联网核实 → 主编裁决,最多 " + p.getMaxRounds() + " 轮。");

            ReActAgent author = buildAuthor(p.getGenre());
            ReActAgent editor = buildEditor(p.getGenre());
            // 核查员的 web_search 每次调用都实时写入事件时间线(暴露它的「思考」:在核实什么)
            ReActAgent factchecker = buildFactChecker((query, preview) ->
            ...
            p.setStatus("DONE");
            repo.save(p);
            log.info("writing done id={} approved={} chars={}", id, approved, finalText.length());
        } catch (Exception e) {
    ...
    private ReActAgent buildAuthor(String genre) {
    ...
    private ReActAgent buildEditor(String genre) {
        return ReActAgent.builder()
                .name("editor")
    ...
    private ReActAgent buildFactChecker(java.util.function.BiConsumer<String, String> searchListener) {
        Toolkit toolkit = new Toolkit();
        toolkit.registerTool(new WebSearchTool(chatModel, searchListener));
        return ReActAgent.builder()
                .name("factchecker")
    ...
    /** 阻塞调用一个 Agent 并取其回复文本。 */
    private String ask(ReActAgent agent, String prompt) {
