...
/**
 * 斯坦福小镇式的<b>自主行动引擎</b>。每 30s 醒来一次(独立 bean,规避 @Scheduled 自调用坑),
...
public class AutonomousRunner {
    ...
    /** 让 actor 自主行动一次并落库。 */
    private void act(AgentEmployee actor, List<AgentEmployee> peers, String modelName) {
        HarnessAgent agent = agentFactory.forEmployee(actor, modelName);
        String prompt = buildPrompt(actor, peers);
    ...
    private String buildPrompt(AgentEmployee actor, List<AgentEmployee> peers) {
        ...
        return "现在是你在小镇里的自由活动时间。你此刻在「" + hereName + "」。\n"
                + "镇上的地点有:" + places + "\n"
                + "镇上还有这些居民,你可以去他们所在的地方找他们聊天:"
                + (others.length() == 0 ? "(暂无)" : others) + "\n"
                + "结合你的记忆、心情和身份,决定接下来做一件事。请只输出一个 JSON 对象,不要多余文字,格式:\n"
